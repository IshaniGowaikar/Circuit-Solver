...
% Populating X matrix 
for ptr = 1:num_Nodes
    X{ptr,1} =strcat('V_',num2str(ptr));%Node Voltages
end
mah=1;
for ptr = (ptr+1):ptr+num_L
    X{ptr,1} =strcat('I_L',num2str(mah));%Current through inductor. adds in new row of X matrix
    mah = mah+1;
end
mah=1;
for ptr = (ptr+1):ptr+num_V
    X{ptr,1} =strcat('I_Vs',num2str(mah));%Current due to independent voltage source. adds in new row of X matrix
    mah = mah+1;
end
mah=1;
for ptr = (ptr+1):ptr+num_VCVS
    X{ptr,1} =strcat('Is_VCVS',num2str(mah));%Current induced due to VCVS. adds in new row of X matrix
    mah = mah+1;
end
mah=1;
for ptr = (ptr+1):ptr+num_CCCS
    X{ptr,1} =strcat('Is_CCCS',num2str(mah));%Current induced due to CCCS. adds in new row of X matrix
    mah = mah+1;
end
mah=1;
% for ptr = (ptr+1):ptr+num_CCVS
%     X{ptr,1} =strcat('Is_CCVS',num2str(mah));%Current induced due to CCCS. adds in new row of X matrix
%     mah = mah+1;
% end
%---------------------------------------------------------------------------------------------------------------------------------
